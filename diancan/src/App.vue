<template>
  <div>
    <!-- 数据加载modal start -->
    <!-- <div v-if="tagModel"> -->
      <div v-if="timeout" id="loadingToast" style="display: block;">
      ...
      <div class="changLanguage">
        <span @click="changLanguage('zh-CN')" v-if="this.$i18n.locale == 'en'">中</span>
        <span @click="changLanguage('en')" v-if="this.$i18n.locale == 'zh-CN'">En</span>
      </div>
      <!-- 中英文切换 end-->
      <v-header :seller="seller" style="width:auto;"></v-header>
      <div class="tab border-1px" v-if="tabShow">
      <!-- <div class="tab border-1px"> -->
        <div v-show="this.$root.paymentTag"> <!-- 直接付款 --><!-- 开启直接付款 先付模式-->
          <router-link to="/pay">
            <div class="paydirectly">
              {{urlParam.tablenum}}{{ $t('lang.table') }}付款
            </div>
          </router-link>
        </div>
        <div v-show="!this.$root.paymentTag"> <!-- 直接付款 --><!-- 未开启直接付款 后付模式-->
          <div class="paydirectly">
            {{urlParam.tablenum}}{{ $t('lang.table') }}
          </div>
        </div>
        <div class="tab-item">
          <router-link to="/goods">{{ $t('lang.menu') }}</router-link>  <!--商品-->
        </div>
        <div class="tab-item">
          <router-link to="/ratings">{{ $t('lang.comment') }}</router-link>   <!--评价-->
        </div>
        <div class="tab-item"></div> <!-- 占位作用 -->
        <div @click="miaofu">
          <!-- <router-link to="/seller">商家</router-link> -->
          <!-- <router-link to="/orderDetail" style="margin-top:4px" class="weui-btn weui-btn_mini weui-btn_primary">我要买单</router-link> -->
          <!-- by xiaozc
          <li>
            <toggle-button :labels="{checked: '有图', unchecked: '无图'}" :width="60" :value="layout == 1 ? true:false" @change="changeLayout()"/>
          </li>
          -->
        </div>
      </div>
      <!-- by wangl -->
      <div class='mymenu'>
        <circle-menu type="middle-around" :number='4' animate="animated jello" mask="black" circle>
          <div type="button" slot="item_btn" @click="circleMenu">
            <img class="button_menu" src="../static/image/wxb.png">
          </div>
          <div  slot="item_1">
            <router-link to="/goods">
              <img class="circle_img" src="../static/image/survey.png" @click = "toggleCircleMenu" />
            </router-link>
            <span class="mysty" style="display:none;">{{ $t('lang.IWantToOrder') }}</span>    <!--我要点餐-->
          </div>
          <div slot="item_2">
            <img class="circle_img" src="../static/image/bussiness-card.png"  @click = "getMyInfo" />
            <span class="mysty" style="display:none;">{{ $t('lang.myInformation') }}</span>
          </div>
          <div slot="item_3">
            <router-link to="/orderList">
              <img class="circle_img" src="../static/image/cart.png" @click = "toggleCircleMenu" />
            </router-link>
            <span class="mysty" style="display:none;">{{ $t('lang.myOrder')}}</span>
          </div>
         <!--  <div slot="item_4">
            <router-link to="/orderDetail">
              <img class="circle_img" src="../static/image/order.png" @click= "toggleCircleMenu" />
            </router-link>
            <span class="mysty">订单详情</span>
          </div> -->
        </circle-menu>
      </div>
      <div class="js_dialog" id="iosDialog2" v-bind:class="{myInfoModal: MyInfo}" style="opacity: 0; display: none;">
          <div class="weui-mask"></div>
          <div class="weui-dialog">
              <div class="weui-dialog__bd">{{ $t('lang.noInformation')}}</div>
              <div class="weui-dialog__ft">
                  <a href="javascript:;" class="weui-dialog__btn weui-dialog__btn_primary" @click = "closeInfo">{{ $t('lang.confirm') }}</a>
              </div>
          </div>
      </div>
    <!-- </div> -->
  ...
  export default {
    ...
    created() {
      ...
      if (this.urlParam.openid) {
        // alert('重新跳转' + location.href);
        this.$root.platformId = this.urlParam.openid;
        // alert('platformId' + this.$root.platformId);
        if (this.$root.platformId.length > 28) {
          this.$root.platformId = this.$root.platformId.substr(0, 28);
        }
        sessionStorage.setItem('platformId', this.$root.platformId);
        location.href = this.redirectUri + '?token=' + localStorage.getItem('token') + '&tablenum=' + localStorage.getItem('tablenum');
        // if (this.$root.platformId.length === 0) {
        //   alert('获取platformid');
        //   this.getPlatformId();
        // }
      }
...
//      console.log('this.seller.name=' + this.seller.name);
      ...
      if (!sessionStorage.getItem('platformId')) {
        // alert('sessionStorage中未存储platformid');
        this.getPlatformId();
      } else {
        this.getMember(); // 查询会员信息
        this.getPaid(false); // 刷新获取上一单支付状态不跳转
      }
    },
    mounted () {
...
// **************************
      ...
      getPlatformId() { // by wangl
        ...
        if (this.$root.clientInfo === 'MicroMessenger') {
          // alert('openid1:' + this.$root.urlParam.openid + 'url::::' + window.location.href);
          if (this.$root.urlParam.openid) { // 1.0扫码进入 不在重复获取openid; by wangl
            // alert('openid2:' + this.$root.urlParam.openid);
      ...
      getOpenId() { // by wangl
        ...
        if (!self.urlParam.code) {
          ...
          axios.get(restUrl).then((res) => {
            ...
            window.localStorage.setItem('repasterptag', res.data.hotel.erptag);
...
// 决定是否请求会员接口 start
// ************************
            for (var i = 0; i < res.data.payconfig.length; i++) {
              if (res.data.payconfig[i].paytype === 'membercard') {
                sessionStorage.setItem('memberPartner', res.data.payconfig[i].partner);
                break;
              }
            }
...
// ************************
            ...
            } else if (payConfigJson.partner === '5icrm') {
              location.href = 'http://www.xiucan.com/api/v2/5icrm/GetOpenid.php'; // GetOpenid    GetOpenidtest
            } else {
      ...
      //     this.layout = 1;
      //   } else if (window.localStorage.getItem('layout')) {
      ...
      getMember() {
        var self = this;
        if (sessionStorage.getItem('memberPartner') && sessionStorage.getItem('payMode') === '0') { // 如果有配置会员支付并且先付模式
          // 查询是否为会员  strat ********************
          var rest = '';
          rest = 'http://www.xiucan.com/api/v2/memberinfo/' + sessionStorage.getItem('memberPartner') + '/' + sessionStorage.getItem('platformId');
          // rest = 'http://www.xiucan.com/api/v2/memberinfo/' + self.$root.memberName + '/odn6QuOCf7IOno1ZlMb-b_fcQVfg';  // 触联
          // rest = 'http://www.xiucan.com/api/v2/memberinfo/' + self.$root.memberName + '/oMSbzt9UmxeUcfJYB0KOckaiN96U';  // 5icrm
          // console.log(rest);
      ...
      getPaid(value) { // 获取上一单支付状态
        if (localStorage.getItem('repasterptag') !== 'ricard' && sessionStorage.getItem('payMode') === '1') {
          var restUrl = 'http://www.xiucan.com/api/v2/getorderdatalist/' + sessionStorage.getItem('platformId') + '/' + sessionStorage.getItem('cid');
          axios.get(restUrl).then((res) => {
            // console.log('获取上一单支付状态' + JSON.stringify(res.data));
            if (res.data && res.data[0].paid === 0) {
              console.log('未支付');
              sessionStorage.setItem('orderId', res.data[0].id);
              this.$root.foodAdd = true;
              this.$root.peopleNum = res.data[0].person_num;
              if (value) {
                this.$router.push({name: 'orderDetail', params: {orderId: res.data[0].orderid}});
              }
            }
          });
        }
