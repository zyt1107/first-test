...
<!--             <div style="margin-left:10px; display:inline-block">
        ...
        <div v-if="!peopleShow">
          <div class="list-header" v-show="!tableShow">
            <h1 class="title">{{ $t('lang.shopCart') }}<span v-if="$root.foodAdd">（加菜）</span></h1>
...
//          return `￥${this.minPrice}元起送`;
    ...
    mounted() {
      ...
      this.$root.eventHub.$on('arriveOrderDetail', (res) => { // 下单成功后到达订单详情页，清空购物车并取消订单处理中提示
        this.orderHandling = false;
...
//          console.log('就餐人数选择完成');
    ...
    methods: {
      memberBalance() {
        var memberBalance = sessionStorage.getItem('memberPartner') === '5icrm' ? this.$root.member.balance : this.$root.member.memberBalance.CardBalance;
      ...
      empty(peopleNumOption) {
        ...
        localStorage.removeItem(goodsKey);  // 清空存贮在loaclstorage中的goods,避免用户在下单或者清空购物车后，退出再次进入页面出现上次数据
      },
      showtablelist() {
...
// ************************************支付方式判断start****************************************
        ...
        if (this.payMethod === 'memberPay') { // 判断支付方式为会员支付 by shaomeng
          ...
          if ((this.$root.member.memberBalance.CardBalance || this.$root.member.balance) < this.totalPrice) {
              this.lessBalanceDialog = true;
              return;
            }
          if (sessionStorage.getItem('memberPartner') === '5icrm') {
...
// 菜品code分别是：${this.selectFoods[0].code}, urlParam.tablenum=${this.tableNum}, urlParam.token=${this.$root.urlParam.token}, embedded=${this.$root.urlParam.embedded} ...
    ...
    */
          ...
          */
          ...
          var self = this;
          // 加菜，（点击订单详情加菜按钮）
          if (self.$root.foodAdd) {
            var foodAddData = {};
            foodAddData.cid = dataJson.cid;
            foodAddData.code = sessionStorage.getItem('orderId');
            foodAddData.order = dataJson.data.order;
            // console.log('加菜数据' + JSON.stringify(foodAddData));
            axios.post('http://www.xiucan.com/api/v2/addtoorder', foodAddData).then((res) => {
              console.log('加菜返回结果' + JSON.stringify(res.data));
              if (res.data.state === 1) {
