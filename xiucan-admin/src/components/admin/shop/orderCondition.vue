<template>
	<div style="width:100%; height:100%; position:absolute">
		...
		<table class="header reporttable" style="z-index:333">
			<thead>
				<tr>
					<td style="width:20%; min-width:41px">日期</td>
					<td style="width:28%">菜品</td>
					<td style="width:12%">数量</td>
					<td style="width:10%; min-width:60px">总金额</td>
					<td style="width:13%; min-width:69px">状态</td>
				</tr>
			</thead>
		</table>
			...
			<table class="body reporttable" ref="commContent">
				<tbody class="content">
					<tr v-for="(item, index) in report"  ref="commaurantItem" :key="index">
						<td style="width:20%; min-width:41px">{{item.time}}</td>
						<td style="width:28%">{{item.name}}</td>
						<td style="width:12%">{{item.orderNum}}</td>
						<td style="width:10%; min-width:60px">{{item.sumMouney}}</td>
						<td style="width:13%; min-width:69px">{{item.type == 0 ? '未支付' : '已支付'}}</td>
					</tr>
				</tbody>
	...
	methods: {
		...
		chooseData() {		// 动态过滤并归类orderdetail数据
			...
			this.orderdetail.forEach((item, index) => {
				...
				reportItem.time = item.servicetime;
				reportItem.name += item.dishinfo.map((food, index) => {
					return food.name;
				});
				reportItem.orderNum += item.dishinfo.map((a, b) => {
					return a.num;
				})[item.dishinfo.length - 1];
				reportItem.sumMouney += item.dishinfo.map((a, b) => {
					return (parseFloat(a.price) * a.num);
				})[item.dishinfo.length - 1];
				reportItem.type = item.paid;
				this.$set(this.report, index, JSON.parse(JSON.stringify(reportItem)));
			});
			this.$nextTick(() => {
				this.initCountscroll();
			});
