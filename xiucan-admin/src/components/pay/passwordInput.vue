    ...
    <div class="modal-wrap"   @click="closePass" id="modal-wrap">
        <div class="input-wrap" @click.stop="">
            <div class="top-wrapper">
                ...
                <div class="tip-count text-center" v-if="tradeAmount">{{ $t('lang.PaymentAmount') }} <a href="javascript:">{{tradeAmount}}</a> {{ $t('lang.yuan') }}</div><!-- 支付金额为 -->
                <!-- 输入框 -->
                ...
                <div class="row" style="padding:0 10px" v-if="findPasswordPath">
                    <router-link :to="findPasswordPath" replace>
                        <a class=" font-small text-right" style="font-size:14px;line-height:30px">{{ $t('lang.forgetPassword') }}?</a>
                    </router-link>
                </div>
            </div>
            <!-- 小键盘 -->
    ...
    export default {
        ...
        data () {
            return {
                passwords : [],
                errorMsg : '',
                title: this.$t('lang.MemberPayPassword')
            }
        },
        methods : {
            inputPasswordHandler : function(num) {
                ...
                if(this.passwords.length === this.passLength) {
                	// console.log('success' + this.passwords)
                    /** 向上冒泡密码输入完成事件 */
                    setTimeout(()=>{
                        this.$root.eventHub.$emit('passwordCompletedInput', this.passwords.join(""))
