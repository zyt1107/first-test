  ...
  <div style="position: absolute;top: -4%;width:100%;">
    ...
    <div class="weui-cells" v-show="!orderDetailShow">
      ...
      <div class="weui-cell" style="background-color: #f8f8f8;padding:5px;"></div>
        ...
        <div class="weui-cell">
            <div class="weui-cell__bd">
                <p v-if="this.$i18n.locale == 'zh-CH'">共{{dishlength}}个菜品</p>
        ...
        <div class="weui-cell" style="background-color: #f8f8f8;padding:5px;" v-if="invoiceShow">
        </div>
    ...
    <a href="javascript:;" v-if="(orderDetail.orderChannel == 'miaodan' || orderDetail.orderChannel == 'paylater') && orderDetail.paid == 0 && repasterptag" v-bind:class="{back_menu_more: payTabShow}" class="weui-btn weui-btn_primary back_menu buttonsetting" @click="miaoDanToPay">{{orderDetail.price}}{{ $t('lang.yuan') }} {{ $t('lang.confirmPay') }}</a><!-- 确认买单 -->
...
export default {
    ...
    activated() {
      ...
      this.$root.eventHub.$emit('arriveOrderDetail'); // 下单成功后到达订单详情页，清空购物车并取消订单处理中提示(shopcart.vue)
      var self = this;
    ...
    methods: {
      ...
      miaoDanToPay() { // 秒付
        ...
        if (this.orderDetail.paymode === 1) {
           orderJson.partner = 'wxpay_xc'; // 合伙人
          if (this.$root.clientInfo === 'AlipayClient') {
